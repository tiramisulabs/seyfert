...
import type { APIGuildWidget, APIGuildWidgetSettings, AuditLogEvent } from '../types';
import type {
...
export function GuildCommunityMethods({ client, guildId }: MethodContext<{ guildId: string }>) {
	return {
		...
		auditLogByUser: (userId: string, query?: Omit<RESTGetAPIAuditLogQuery, 'user_id'>): Promise<RESTGetAPIAuditLogResult> =>
			client.guilds.audit.byUser(guildId, userId, query),
		auditLogByAction: (
		...
		editWelcomeScreen: (body: RESTPatchAPIGuildWelcomeScreenJSONBody, reason?: string): Promise<GuildWelcomeScreenStructure> =>
		...
		editWidgetSettings: (body: RESTPatchAPIGuildWidgetSettingsJSONBody, reason?: string): Promise<APIGuildWidgetSettings> =>
			client.guilds.widget.edit(guildId, body, reason),
		...
		editIncidents: (body: RESTPutAPIGuildIncidentActionsJSONBody, reason?: string): Promise<RESTPutAPIGuildIncidentActionsResult> =>
			client.guilds.incidents(guildId, body, reason),
	};
}
