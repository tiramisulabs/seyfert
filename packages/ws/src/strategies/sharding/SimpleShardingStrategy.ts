...
export class SimpleShardingStrategy implements IShardingStrategy {
	...
	public async spawn(shardIds: number[]) {
		...
		for (const shardId of shardIds) {
			...
			for (const event of Object.values(WebSocketShardEvents)) {
				shard.on(event, (payload) =>
					// @ts-expect-error
					this.manager.emit(event, { ...payload, shardId }),
				);
			}
	...
	public async fetchStatus() {
		const map = new Map<number, WebSocketShardStatus>();
		[...this.shards.values()].map((shard, i) => map.set(i, shard.status));
		return map;
	}
}
