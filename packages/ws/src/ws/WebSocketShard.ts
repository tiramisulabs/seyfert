...
export class WebSocketShard extends AsyncEventEmitter<WebSocketShardEventsMap> {
	...
	private readonly timeouts = new Map<WebSocketShardEvents, NodeJS.Timeout>();
	...
	public async connect() {
		...
		const { version, encoding } = this.strategy.options;
		...
		if (--this.sendRateLimitState.remaining <= 0) {
			...
			if (this.sendRateLimitState.resetAt > now) {
				...
				const interrupted = await Promise.race([
					...
					once(this, WebSocketShardEvents.Closed, {
	...
	private async onMessage(data: RawData, isBinary: boolean) {
		...
		switch (payload.op) {
			...
			case GatewayOpcodes.HeartbeatAck: {
				...
				this.#ping = ackAt - this.lastHeartbeatAt;
