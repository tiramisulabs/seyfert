...
import { range, DefaultWebSocketManagerOptions, type Encoding } from "../utils/constants.js";
...
export class WebSocketManager extends AsyncEventEmitter<ManagerShardEventsMap> {
	...
	public async fetchGatewayInformation(force = false) {
		...
		const data = await this.options.rest.get<RESTGetAPIGatewayBotResult>(Routes.gatewayBot());
