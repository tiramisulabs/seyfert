			...
			switch (message.op) {
				...
				case WorkerSendPayloadOp.SessionInfoResponse: {
					...
					const session = {
						...
						session: {
							...message.session,
							sequence: message.session.sequence + 1,
						},
					} as WorkerReceivePayload;
...
test("spawn, connect, send a message, session info, and destroy", async () => {
	...
	const strategy = new WorkerShardingStrategy(manager, {
		shardsPerWorker: "all",
	});
	...
	expect(mockConstructor).toHaveBeenCalledWith(
		...
		expect.objectContaining({
			workerData: expect.objectContaining({ shardIds: [0, 1] }),
		}),
	);
	...
	expect(mockUpdateSessionInfo).toHaveBeenCalledWith(0, {
		...sessionInfo,
		sequence: sessionInfo.sequence + 1,
	});
