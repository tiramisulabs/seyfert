...
import { WebSocketManager, WebSocketShardStatus, type IShardingStrategy } from "../../dist/index.js";
...
test("fetch gateway information", async () => {
	...
	rest.setToken("A-Very-Fake-Token");
	rest.cRest.setAgent(mockAgent);
...
test("strategies", async () => {
	class MockStrategy implements IShardingStrategy {
		fetchStatus(): Awaitable<Map<number, WebSocketShardStatus>> {
	...
	const send: GatewaySendPayload = {
		...
		// @ts-expect-error
		d: { guild_id: "859477035457970266", limit: 0 },
	};
